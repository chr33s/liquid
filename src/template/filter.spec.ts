...
describe('filter', function () {
  ...
  it('should call filter impl with correct this', async function () {
    const spy = vi.fn(function* (this: any, valStr, diff): Generator<string> {
  ...
  it('should render a simple filter', async function () {
    expect(
      await toPromise(
        new Filter({ name: 'upcase', args: [] } as any, (x: string) => x.toUpperCase(), liquid).render('foo', ctx)
      )
    ).toBe('FOO')
  })
  it('should reject promise when filter throws', async function () {
    const filter = new Filter(
      { name: 'foo', args: [] } as any,
      function* () {
        throw new Error('intended')
      },
      liquid
    )
    await expect(toPromise(filter.render('foo', ctx))).rejects.toMatchObject({
      message: 'intended'
    })
  })
  it('should render filters with argument', async function () {
    ...
    expect(
      await toPromise(
        new Filter({ name: 'add', args: [two] } as any, (a: number, b: number) => a + b, liquid).render(3, ctx)
      )
    ).toBe(5)
  })
  ...
  it('should pass Objects/Drops as it is', async function () {
    ...
    expect(
      await toPromise(
        new Filter({ name: 'name', args: [] } as any, (a: any) => a.constructor.name, liquid).render(new Foo(), ctx)
      )
    ).toBe('Foo')
  })
  ...
  it('should support key value pairs', async function () {
    ...
    expect(
      await toPromise(
        new Filter(
          { name: 'add', args: [['num', two]] } as any,
          (a: number, b: number[]) => b[0] + ':' + (a + b[1]),
          liquid
        ).render(3, ctx)
      )
    ).toBe('num:5')
