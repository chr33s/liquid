...
const formatCodes: Record<string, FormatCodeHandler> = {
  ...
  c: (d: LiquidDate) => strftime(d, '%a %b %e %H:%M:%S %Y'),
  C: (d: LiquidDate) => century(d),
  D: (d: LiquidDate) => strftime(d, '%m/%d/%y'),
  F: (d: LiquidDate) => strftime(d, '%Y-%m-%d'),
  ...
  r: (d: LiquidDate) => strftime(d, '%I:%M:%S %p'),
  R: (d: LiquidDate) => strftime(d, '%H:%M'),
  s: (d: LiquidDate) => Math.floor(d.getTime() / 1000),
  S: (d: LiquidDate) => d.getSeconds(),
  T: (d: LiquidDate) => strftime(d, '%H:%M:%S'),
  u: (d: LiquidDate) => d.getDay() || 7,
  U: (d: LiquidDate) => getWeekOfYear(d, 0),
  v: (d: LiquidDate) => strftime(d, '%e-%^b-%Y'),
  V: (d: LiquidDate) => isoWeek(d),
  w: (d: LiquidDate) => d.getDay(),
  W: (d: LiquidDate) => getWeekOfYear(d, 1),
  x: (d: LiquidDate) => strftime(d, '%m/%d/%y'),
  X: (d: LiquidDate) => strftime(d, '%H:%M:%S'),
