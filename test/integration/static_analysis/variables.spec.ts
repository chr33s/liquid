...
describe('Variable analysis', () => {
  ...
  it.each(['include', 'render', 'layout'])('should stop recursion at the first %s tag', async tag => {
    const engine = new Liquid({ templates: { a: `{% ${tag} "a" %}{{ x }}` } })
  ...
  it.each(['include', 'render', 'layout'])('should report dynamic %s filenames', async tag => {
    const engine = new Liquid()
  ...
  it('should load child templates asynchronously', async () => {
    const source = [
      '{% if a %}',
      '  {% for x in b %}',
      '    {% unless x == y %}',
