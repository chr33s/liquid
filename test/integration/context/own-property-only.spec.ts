...
describe('ownPropertyOnly / inherited array indices', function () {
  ...
  it.each(cases)('%s', async function (src, scopeFn, expected) {
    const ctx = pollutedArrays()
    try {
      expect(await engine.parseAndRender(src, scopeFn(ctx))).toBe(expected)
    } finally {
      ctx.cleanup()
    }
  })
  it('still allows array length and size', async function () {
    const { a, cleanup } = pollutedArrays()
    try {
      expect(await engine.parseAndRender('{{ a.size }}', { a })).toBe('1')
      const arr = [1, 2]
      expect(await engine.parseAndRender('{{ arr | first }}', { arr })).toBe('1')
      expect(await engine.parseAndRender('{{ arr[-1] }}', { arr })).toBe('2')
    } finally {
      cleanup()
    }
  })
