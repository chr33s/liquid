...
describe('LiquidOptions#fs', function () {
  ...
  const fs = {
    ...
    existsSync: () => {
      throw new Error('removed existsSync invoked')
    },
    readFile: (x: string) => Promise.resolve(`content for ${x}`),
    readFileSync: () => {
      throw new Error('removed readFileSync invoked')
    },
  ...
  it('should render relative in-memory templates', async () => {
    const engine = new Liquid({
      templates: {
    ...
    expect(await engine.renderFile('views/entry')).toEqual('header footer')
  })
