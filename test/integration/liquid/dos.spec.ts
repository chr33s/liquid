...
describe('DoS related', function () {
  ...
  describe('#outputLengthLimit', () => {
    ...
    it('should enforce outputLengthLimit in Promise rendering', async () => {
      const liquid = new Liquid({ outputLengthLimit: 5 })
      await expect(async () => await liquid.parseAndRender('{% for i in (1..100) %}{{i}}{% endfor %}')).rejects.toThrow(
    ...
    it('should enforce outputLengthLimit in stream render', async () => {
      ...
      const stream = liquid.renderToStream(tpl)
      await expect(drainStream(stream)).rejects.toThrow('output length limit exceeded')
    })
  })
  describe('#maxDepth', () => {
  ...
  describe('strip_html ReDoS', () => {
    ...
    it('should handle many unclosed <script openers in linear time', async () => {
      const liquid = new Liquid()
      const payload = '<script'.repeat(50000)
      expect(await liquid.parseAndRender('{{ x | strip_html }}', { x: payload })).toBe(payload)
    }, 1000)
    it('should handle many unclosed <style openers in linear time', async () => {
      const liquid = new Liquid()
      const payload = '<style'.repeat(50000)
      expect(await liquid.parseAndRender('{{ x | strip_html }}', { x: payload })).toBe(payload)
    }, 1000)
    it('should handle <script openers that have > but no </script> in linear time', async () => {
      const liquid = new Liquid()
      const payload = '<script>foo'.repeat(50000)
      expect(await liquid.parseAndRender('{{ x | strip_html }}', { x: payload })).toBe('foo'.repeat(50000))
    }, 1000)
  })
})
