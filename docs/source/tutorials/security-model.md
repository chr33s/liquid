...
{@link LiquidOptions.maxDepth | maxDepth} limits how deeply `{% render %}`, `{% include %}`, and `{% layout %}` can nest. Defaults to `128`. This limit applies to asynchronous rendering as a DoS guard.
